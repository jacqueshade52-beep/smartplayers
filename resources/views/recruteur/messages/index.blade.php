...
<div class="bg-slate-50 min-h-screen pt-32 pb-20">
    <div class="max-w-7xl mx-auto px-6 h-[calc(100vh-160px)] min-h-[600px]">
        <!-- Header -->
        <div class="mb-6">
            <a href="{{ route('recruteur.dashboard') }}" class="text-sm text-slate-400 hover:text-benin-red inline-block mb-1"><i class="fas fa-arrow-left mr-1"></i> Dashboard</a>
            <h1 class="text-2xl font-extrabold text-benin-dark flex items-center gap-2">
                <i class="fas fa-comment-dots text-slate-300"></i> Boîte de Réception
            </h1>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 flex overflow-hidden h-full reveal">
            ...
            <div class="w-full md:w-80 lg:w-96 border-r border-slate-100 flex flex-col h-full bg-slate-50/50">
                ...
                <div class="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
                    ...
                    <button onclick="loadConversation({{ $conv['user']->id }})" class="w-full text-left p-4 rounded-2xl hover:bg-white hover:shadow-sm transition-all flex gap-3 group">
                    ...
                    <div class="p-8 text-center text-slate-400">
                        ...
                        <p class="text-sm">Votre messagerie est vide.</p>
                    </div>
                    @endforelse
                </div>
            </div>
        ...
        <div class="h-20 border-b border-slate-100 px-6 flex items-center justify-between bg-white z-10 shrink-0">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-benin-green font-semibold flex items-center gap-1"><span class="w-1.5 h-1.5 rounded-full bg-benin-green"></span> En ligne récemment</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 border-t border-slate-100 bg-white shrink-0">
            <form id="reply-form" onsubmit="sendMessage(event, ${user.id})" class="flex items-end gap-3 bg-slate-100 rounded-3xl p-2 pl-4 focus-within:ring-2 focus-within:ring-slate-300 transition-all">
                ...
                <div class="flex gap-1 shrink-0 pb-1">
                    <button type="button" class="w-8 h-8 rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-200 flex items-center justify-center transition-colors"><i class="fas fa-paperclip"></i></button>
                    <button type="submit" class="w-8 h-8 rounded-full bg-benin-dark text-white shadow-md flex items-center justify-center hover:bg-black transition-colors transform hover:scale-105"><i class="fas fa-paper-plane text-xs"></i></button>
                </div>
            </form>
        </div>
    `;
